...
// Building the Modal component on top of Radix UI's Dialog primitive.
// This ensures our modals are fully accessible out-of-the-box (keyboard navigation, focus trapping, etc.).

const Modal = DialogPrimitive.Root;
const ModalTrigger = DialogPrimitive.Trigger;

const ModalPortal = ({ className, ...props }) => (
...
const ModalContent = React.forwardRef(({ className, children, ...props }, ref) => (
  <ModalPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-primary-700 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-neutral-100 data-[state=open]:text-neutral-500">
        <X className="h-4 w-4" />
