...
const Stage = ({ number, text, status }) => {
  ...
  return (
    <div className="relative group">
      <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs transition-all duration-300 shadow-sm hover:shadow-md ${statusStyles[status]}`}>
        ...
        <span className="whitespace-nowrap text-xs font-medium">{text}</span>
      </div>
      
      {/* Tooltip */}
      <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-slate-800 dark:bg-slate-900 text-white text-xs rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-20 shadow-lg max-w-xs">
        {tooltips[text]}
...
export default function ProgressIndicator({ currentStage }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-2 w-full overflow-hidden">
      {stages.map((stage, index) => (
        <div key={stage} className="flex-shrink-0">
          <Stage number={index + 1} text={stage} status={getStatus(index)} />
        </div>
      ))}
    </div>
  );
