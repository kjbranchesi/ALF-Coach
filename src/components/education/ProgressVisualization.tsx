      ...
      <div className="space-y-4">
        {stages.map((stage, index) => (
          <div key={stage.id} className={`border rounded-lg p-4 transition-all duration-200
            ${stage.status === 'in-progress' ? 'border-primary-300 bg-primary-50' :
              stage.status === 'completed' ? 'border-success-200 bg-success-50' :
                ...
                <div>
                  ...
                  <span className={`text-xs
                    ${stage.status === 'completed' ? 'text-success-600' :
                      stage.status === 'in-progress' ? 'text-primary-600' :
                      'text-gray-500'}`}>
                ...
                {stage.substeps.map((substep, subIndex) => (
                  <div key={subIndex} className="flex items-center gap-2 text-sm">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center
                      ${substep.completed ? 'border-success-500 bg-success-500' : 'border-gray-300'}`}>
