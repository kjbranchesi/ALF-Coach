...
/**
 * MainWorkspace is the central hub for all in-project activities.
 * It uses a tabbed interface to switch between the AI chat and the project draft.
 */
export default function MainWorkspace() {
  const { selectedProjectId, navigateTo } = useAppContext();
  const [project, setProject] = useState(null);
  const [isLoading, setIsLoading] = useState(true);
  const [activeTab, setActiveTab] = useState('chat'); // 'chat' or 'draft'
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        console.error("Workspace: Project not found!");
        navigateTo('dashboard');
      }
      setIsLoading(false);
    });
  ...
  if (isLoading || !project) {
    return (
      <div className="flex items-center justify-center h-full">
        <h1 className="text-2xl font-bold text-purple-600 animate-pulse">Loading Workspace...</h1>
      </div>
    );
  }
