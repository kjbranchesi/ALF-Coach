...
export function buildFutureFoodHero(userId: string): SampleBlueprint {
  const heroData = getHeroProject('hero-future-food');

  if (!heroData) {
    throw new Error('Hero project data not found for hero-future-food');
  }
  ...
  return {
    ...
    metadata: {
      isHeroProject: true,
      heroProjectId: 'hero-future-food',
      tags: ['sustainability', 'food-systems', 'circular-economy', 'nutrition', 'waste-reduction'],
      difficulty: 'intermediate',
      estimatedHours: 60,
      studentRange: '25-30',
      techRequired: 'low',
      spaceRequired: 'classroom + cafeteria'
    }
  };
}