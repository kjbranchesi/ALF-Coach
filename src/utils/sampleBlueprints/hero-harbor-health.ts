...
export function buildHarborHealthHero(userId: string): SampleBlueprint {
  const heroData = getHeroProject('hero-harbor-health');

  if (!heroData) {
    throw new Error('Hero project data not found for hero-harbor-health');
  }
  ...
  return {
    ...
    metadata: {
      isHeroProject: true,
      heroProjectId: 'hero-harbor-health',
      tags: ['marine-science', 'environmental-monitoring', 'water-quality', 'citizen-science', 'advocacy'],
      difficulty: 'advanced',
      estimatedHours: 100,
      studentRange: '20-30',
      techRequired: 'high',
      spaceRequired: 'classroom + laboratory + waterfront access'
    }
  };
}