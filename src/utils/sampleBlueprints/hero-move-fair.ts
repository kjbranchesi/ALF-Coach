...
export function buildMoveFairHero(userId: string): SampleBlueprint {
  const heroData = getHeroProject('hero-move-fair');

  if (!heroData) {
    throw new Error('Hero project data not found for hero-move-fair');
  }
  ...
  return {
    ...
    metadata: {
      isHeroProject: true,
      heroProjectId: 'hero-move-fair',
      tags: ['urban-mobility', 'transportation', 'equity', 'data-science', 'community'],
      difficulty: 'advanced',
      estimatedHours: 80,
      studentRange: '25-30',
      techRequired: 'moderate',
      spaceRequired: 'classroom + field work'
    }
  };
}