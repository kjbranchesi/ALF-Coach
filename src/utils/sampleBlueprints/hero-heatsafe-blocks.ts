...
export function buildHeatSafeBlocksHero(userId: string): SampleBlueprint {
  const heroData = getHeroProject('hero-heatsafe-blocks');

  if (!heroData) {
    throw new Error('Hero project data not found for hero-heatsafe-blocks');
  }
  ...
  return {
    ...
    metadata: {
      isHeroProject: true,
      heroProjectId: 'hero-heatsafe-blocks',
      tags: ['urban-planning', 'climate-action', 'community-health', 'data-science', 'civic-engagement'],
      difficulty: 'intermediate',
      estimatedHours: 72,
      studentRange: '25-30',
      techRequired: 'medium',
      spaceRequired: 'classroom + community spaces'
    }
  };
}