...
export function buildAccessAbilityAIHero(userId: string): SampleBlueprint {
  const heroData = getHeroProject('hero-accessability-ai');

  if (!heroData) {
    throw new Error('Hero project data not found for hero-accessability-ai');
  }
  ...
  return {
    ...
    metadata: {
      isHeroProject: true,
      heroProjectId: 'hero-accessability-ai',
      tags: ['accessibility', 'ai', 'inclusion', 'wcag', 'disability-rights', 'edtech'],
