...
export function buildPlayableCityHero(userId: string): SampleBlueprint {
  const heroData = getHeroProject('hero-playable-city');

  if (!heroData) {
    throw new Error('Hero project data not found for hero-playable-city');
  }
  ...
  return {
    ...
    metadata: {
      isHeroProject: true,
      heroProjectId: 'hero-playable-city',
      tags: ['urban-design', 'game-design', 'placemaking', 'community-engagement', 'creative-technology'],
      difficulty: 'intermediate',
      estimatedHours: 80,
      studentRange: '20-30',
      techRequired: 'medium',
      spaceRequired: 'classroom + public spaces'
    }
  };
}